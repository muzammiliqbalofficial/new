...
import { Search, ShoppingBag, Phone, Menu, X, Truck } from 'lucide-react';
...
export default function Header() {
  ...
  // Lock background body scroll when mobile drawer is open to prevent page overlap/scrolling
  useEffect(() => {
    if (isMobileMenuOpen) {
  ...
  return (
    <>
      <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-charcoal-border/70 shadow-xs">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16 sm:h-20 gap-4">
            ...
            {/* Brand Logo & Name (tinykids.pk + Tagline: Premium Kids Store) */}
            <div className="flex items-center">
              <Link href="/" className="flex items-center space-x-3 group">
                <div className="relative w-10 h-10 sm:w-12 sm:h-12 rounded-2xl overflow-hidden bg-white shadow-soft border border-charcoal-border/60 transition-transform group-hover:scale-105 flex-shrink-0">
                  <Image src="/logo.png" alt="tinykids.pk Logo" fill className="object-cover" priority />
                </div>
                <div className="flex flex-col">
                  <span className="font-extrabold text-lg sm:text-2xl text-charcoal tracking-tight leading-none lowercase group-hover:text-brand transition-colors">
      ...
      {/* Standalone Solid Mobile Drawer (Outside Header to fix Safari backdrop-blur overlap) */}
      {isMobileMenuOpen && (
        <div className="fixed inset-0 z-[9999] lg:hidden">
          ...
          {/* Fully Opaque Solid White Drawer Panel */}
          <div className="fixed inset-y-0 left-0 max-w-xs w-full bg-white z-[10000] shadow-2xl flex flex-col justify-between p-6 overflow-y-auto">
            <div className="space-y-6">
              {/* Drawer Header */}
              <div className="flex items-center justify-between border-b border-charcoal-border/60 pb-4">
                <div className="flex items-center space-x-2.5">
                  <div className="relative w-8 h-8 rounded-xl overflow-hidden bg-white shadow-xs border border-charcoal-border/50 flex-shrink-0">
                    <Image src="/logo.png" alt="tinykids.pk" fill className="object-cover" />
                  </div>
                  <div className="flex flex-col">
                    <span className="font-extrabold text-base text-charcoal lowercase leading-tight">
                      tinykids<span className="text-brand">.pk</span>
                    </span>
